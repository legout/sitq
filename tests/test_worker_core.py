...
async def test_worker_logging():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        test_logger = logging.getLogger('test.worker')
        ...
        # Enqueue tasks
        task_queue = TaskQueue(backend, serializer)
        success_id = await task_queue.enqueue(success_task)
        error_id = await task_queue.enqueue(error_task)
        
        # Start worker with test logger
        worker = Worker(backend, serializer, logger=test_logger)
        await worker.start()
